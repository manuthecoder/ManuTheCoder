  ...
  xhttp.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
      ...
      document.getElementById(
        "articles"
      ).innerHTML = `<div class="row"> <div class="col s7 m12" id="articlesContainer"></div><div class="col s3 m12" id="tagContainer"><h4>Tags</h4></div></div>`;
...
<a href="${articles[i].url}" target="_blank" class="card content">${
            articles[i].title
          }</a>
</div>
`;
      ...
      document.getElementById(
        "articlesContainer"
      ).innerHTML += `<button onclick="this.remove();\$('.hide').removeClass('hide')" style="width: 100%;display: block;background: rgba(200, 200, 200, .2);border-radius: 2px;box-shadow: none !important;color: gray !important">View More</button>`;
      blog.tags = blog.tags.split(",");
      blog.tags = [...new Set(blog.tags)];
      document.getElementById(
        "tagContainer"
      ).innerHTML = `<div class="chip" onclick="load_articles('')">🔃</div>`;
      blog.tags.forEach((data) => {
        if (data.trim() !== "") {
          document.getElementById(
            "tagContainer"
          ).innerHTML += `<div class='chip ${
            data.trim() == tag.trim() ? "active" : ""
          }' onclick="load_articles(this.innerText)">${data}</div>`;
        }
      });
    }
  };
